...
if cap:
    # Capture path
    cap_trajectory = np.genfromtxt('path/metalFrame/metalFrame_capture_spacing15_macro.csv', delimiter=',') #cmd_0, 20220721_capture_modify35, grinding_pose, path_20220623-2, two_grinding_pose, geometry, path_grinding, three_grinding_pose
    ...
    pose = [1.3496422652236157, -0.5497141274793291, -2.455721694439394, 1.4843550932907825, 1.5915318664201388, -2.815100289222657] # T_faucet
    # pose = [-1.2455958764311132, 0.8997043203638077, 0.8217772597293986, 2.9677567640085534, -1.5723699019253263, -3.689431237434805]  # metalFrame macro-lens
    ...
    if Execute_preproduct_plan:
        for idx in range(48):
            ...
            req.plan_name = 'plan_path/Capture_plan_20220905' # file name of plan #plan_path/Capture_plan_metalFrame_spacing15_macro_slow,  Capture_plan_metalFrame_spacing15 # Capture_plan_20220905 # Capture_plan_metalFrame_spacing15_macro_fast
            req.plan_index = idx
            if idx==0:
        ...
        for idx,path in enumerate(cap_trajectory):
            pose = [float(path[2])/1000, float(path[3]/1000), float(path[4])/1000, float(path[5])/180*pi, float(path[6])/180*pi, float(path[7])/180*pi]
            print("idx:", idx)
            print("pose:", pose)
            req.goal_format = goal_format
...
if grind:
    ...
    # init_position = [-1.681586224987174, -0.42498395, -1.4227018, 0.41363449, -1.4904326445, 1.494597162] # cylinder grinding
    # init_position = [0.44807, 0.53633, 0.44597, 3.099181152766331, 0.11526539202438, -1.567131135365709] #TCP test
    ...
    # Receive data when no DIO with mini-robot
    if testing:
    ...
    # Receive data by DIO with mini-robot 
    if not sim:
    ...
    motion_type = 'line' # "PTP" for plan on joint space, and "line" for planning on Cartesian space
    print('start grinding path')
    # grinding_path_file = 'path/polishing/new/plate_wide_10deg_interp.csv' # ReGrinding_Cmd_20220907.txt, path/20220723_grinding_big0.csv, path/grinding/plate_grinding_1018.csv
    grinding_path_file = 'path/polishing/new/cylinder/cylinder_d30_quater_modify.csv'
    ...
    # pub_motion_type.publish('PTP')
    grinding_times = []
    Execute_preproduct_plan = False
    if Execute_preproduct_plan:
        grind_plan_name = 'plan_path/polishing/10deg_v0.03_wide'
        ...
        for idx in range(plan_len):
            ...
            grinding_times.append(use_time)
        ...
        if motion_type == 'PTP':
            ...
            ## for cylinder grinding
            count = 0
            for i in range(12): #8 for d40, 
                # break
                pose = []
                if (i-2)%4==0:
                    # break
                    velocity_scale = 0.02348912
                    for j in range(9): #6 for d40
                        path = grinding_trajectory[i + 8*count + j] #5 for d40
                        pose += [float(path[2])/1000, float(path[3]/1000), float(path[4])/1000, float(path[5])/180*pi, float(path[6])/180*pi, float(path[7])/180*pi]
                    count += 1
                else:
                    velocity_scale = 0.03
                    path = grinding_trajectory[i + 8*count]
                    pose = [float(path[2])/1000, float(path[3]/1000), float(path[4])/1000, float(path[5])/180*pi, float(path[6])/180*pi, float(path[7])/180*pi]
                # velocity_scale = 0.03
                # path = grinding_trajectory[i]
                # pose = [float(path[2])/1000, float(path[3]/1000), float(path[4])/1000, float(path[5])/180*pi, float(path[6])/180*pi, float(path[7])/180*pi]
                # print(pose)
                req.goal_format = goal_format
                req.motion_type = motion_type
            ...
            #         velocity_scale = 0.01
            #         for j in range(10):
            #             path = grinding_trajectory[i + 9*count + j]
            #             pose += [float(path[2])/1000, float(path[3]/1000), float(path[4])/1000, float(path[5])/180*pi, float(path[6])/180*pi, float(path[7])/180*pi]
            #         count += 1
            #     else:
            #         velocity_scale = 0.03
            #         path = grinding_trajectory[i + 9*count]
            #         pose = [float(path[2])/1000, float(path[3]/1000), float(path[4])/1000, float(path[5])/180*pi, float(path[6])/180*pi, float(path[7])/180*pi]
            #     # velocity_scale = 0.03
            #     # path = grinding_trajectory[i]
            #     # pose = [float(path[2])/1000, float(path[3]/1000), float(path[4])/1000, float(path[5])/180*pi, float(path[6])/180*pi, float(path[7])/180*pi]
...
np.savetxt('grinding_times.csv', grinding_times, delimiter=',')
print("Save path_time")
