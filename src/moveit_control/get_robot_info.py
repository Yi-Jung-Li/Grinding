...
parser.add_argument("-dt", "--sampling_time", type=float, default=0.03)
...
while state != 2:
    ...
    while state == 1:
        ...
        # joint_state = rospy.wait_for_message('/joint_states', JointState)
        # JointData = list(joint_state.position)
        # velocity = list(joint_state.velocity)
        # effort = list(joint_state.effort)
        # # End_effect = pose.pose


        # # robot_state = group.get_current_state()
        # # JointData = list(robot_state.joint_state.position)
        # link_name = "link_6"
        # header = Header()
        ...
        last_time = T
        print(T-t1)
        JointData = list(feedback_states.joint_pos)
        velocity = list(feedback_states.joint_vel)
        effort = list(feedback_states.joint_tor)
        end_effect = list(feedback_states.tool_pose)
        end_effect_velocity = list(feedback_states.tcp_speed)
        ...
        data = JointData+velocity+effort+end_effect+end_effect_velocity+[T]+[idx] # JointData(6 joints) + end_effect(x,y,z) + index
        # data = JointData+end_effect+[T]+[idx]
